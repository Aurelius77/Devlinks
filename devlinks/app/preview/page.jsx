...
export default function Preview() {
  ...
  const handleShare = async () => {
    const handle = userName || 'developer';
    ...
    try {
      // Call backend Server Action to save/publish user in MongoDB
      const res = await run(
        userFirstName || 'Dev',
        userLastName || 'User',
        userImage || '',
      ...
      console.error(err);
      // Fallback copy if server action fails or offline
      if (navigator.clipboard) {
        await navigator.clipboard.writeText(profileUrl);
  ...
  return (
    <div className="min-h-screen bg-[#0b0f19] flex flex-col relative overflow-hidden">
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-6 relative z-10">
        <PhoneMockup userData={userData} interactive={true} className="animate-float" />
      </main>
    </div>
  );
}